...
def create_matrix_of_document_idf(inverted_index,tokens_per_document_dict,input_query,idf_of_all_index_terms,qid):
    ...
    tfidf_cal_for_each_doclist=[]
    tfidf_cal_for_each_docdict={}
    sorted_tfidf_cal_for_each_docdict = {}
    tfidf_cal_for_each_doclist=[sum(row) for row in doc_idf_matrix]
    key_value=1

    for each in tfidf_cal_for_each_doclist:
        tfidf_cal_for_each_docdict[key_value]=each
        key_value+=1

    #print "tfidf_cal_for_each_docdict"
    #print tfidf_cal_for_each_docdict

    sorted_tfidf_cal_for_each_doclist=sorted(tfidf_cal_for_each_docdict.items(), key=operator.itemgetter(1))

    #print "sorted_tfidf_cal_for_each_doclist"
    #print sorted_tfidf_cal_for_each_doclist

    eachterm=len(sorted_tfidf_cal_for_each_doclist)
    rank=1

    while eachterm > 0 and rank <= 100:
        print str(qid) + " Q0 " +" "+"CACM-"+str(sorted_tfidf_cal_for_each_doclist[eachterm-1][0]) +" "+ str(rank) +" "+ str(sorted_tfidf_cal_for_each_doclist[eachterm-1][1]) +" "+ "tf.idf-Algorithm"
        eachterm -= 1
        rank=rank+1
